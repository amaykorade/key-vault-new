    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in pb-10">
      ...
      <Card className="hover-lift border-emerald-500/30">
        ...
        <CardContent className="space-y-4">
          ...
          {/* Local development via CLI (package + setup commands) */}
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <p className="text-xs font-semibold text-purple-100">
                  Use secrets locally via CLI (recommended for dev)
                </p>
              </div>
              <button
                onClick={() => navigate(ROUTES.CLI_GUIDE)}
                className="px-2.5 py-1 text-[11px] rounded-md border border-purple-500/40 text-purple-100 bg-purple-500/10 hover:bg-purple-500/20 transition-colors"
              >
                Full CLI guide →
              </button>
            </div>
            <ol className="space-y-1.5 text-[11px] text-purple-100 list-decimal list-inside">
              <li>
                <code className="bg-gray-900 px-1 py-0.5 rounded border border-gray-800 mr-1">npm install -g @keyvault/cli</code>
                <span>Install the Key Vault CLI package globally.</span>
              </li>
              <li>
                <code className="bg-gray-900 px-1 py-0.5 rounded border border-gray-800 mr-1">keyvault login</code>
                <span>Authenticate once via browser.</span>
              </li>
              <li>
                <code className="bg-gray-900 px-1 py-0.5 rounded border border-gray-800 mr-1">keyvault setup</code>
                <span>Pick your project / environment / folder for this repo.</span>
              </li>
              <li>
                <code className="bg-gray-900 px-1 py-0.5 rounded border border-gray-800 mr-1">keyvault run -- npm start</code>
                <span>
                  Run your app with secrets injected as environment variables (
                  <code className="bg-gray-900 px-1 py-0.5 rounded text-[10px] ml-1">process.env.SECRET_NAME</code>
                  ).
                </span>
              </li>
            </ol>

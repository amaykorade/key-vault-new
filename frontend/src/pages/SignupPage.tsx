...
export function SignupPage() {
  ...
  const manualSignupDisabled = true;
  ...
  const onGitHubSignup = () => {
    window.location.href = `${API_BASE_URL.replace(/\/$/, '')}/auth/github`;
  };
        ...
        <Card className="border-gray-800 bg-gray-900/70 backdrop-blur-sm">
          <CardHeader className="border-b border-gray-800">
            ...
            <CardDescription className="text-gray-400">
              Email/password sign up is temporarily disabled. Please continue with Google or GitHub below.
            </CardDescription>
          </CardHeader>
          <CardContent className="pt-6">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              {manualSignupDisabled && (
                <div className="rounded-lg border border-yellow-500/30 bg-yellow-500/10 p-3 text-xs text-yellow-100 mb-2">
                  Creating accounts via email + password is currently disabled while we roll out OAuth-based sign up.
